(function (global, factory) {
    ...
    /**
     * @file Adds [centerblock] to bbcode; 50% is default
     * @example [centerblock=OPTION]content[/centerblock]
     */
    const centerblock = (node) => {
    ...
    /**
     * @file Adds [check] to bbcode; dot is default
     * @example [check=OPTION]content[/check]
     */
    const check = (node) => {
     ...
     */
    function parseHeight$1(heightValue) {
      const maxHeight = 700;
      const parsedHeight =
     ...
     */
    const heightrestrict = (node) => {
      const attrs = preprocessAttr(node.attrs)._default;
      const heightInput = parseHeight$1(attrs).toString();
     ...
     * @exmaple [progress=percentageInput]content[/progress]
     */
    const progress = (node) => {
        const percentageInput = preprocessAttr(node.attrs)._default;
        return {
          tag: "div",
          attrs: {
            class: "bb-progress",
          ...
          content: [
            ...
            },
            {
              ...
              attrs: {
                ...
                style: `width: calc(${percentageInput}% - 6px)`
              }
            },
            {
              tag: "div",
     ...
     * @exmaple [thinprogress=percentageInput]content[/thinprogress]
     */
    const thinprogress = (node) => {
      const percentageInput = preprocessAttr(node.attrs)._default;
      return {
        tag: "div",
        attrs: {
            ...
            content: [
              {
                ...
                content: preprocessAttr(node.attrs)._default || "Recipient"
              }
            ]
          }
        ),
      message: (node) =>
        // const senderAttrs = preprocessAttr(node.attrs)._default;
        // console.log("TEST",senderAttrs);
        // const senderStyle = senderAttrs.startsWith("me") ? "bb-message-me" : "bb-message-them";
        toNode(
          "div",
          {

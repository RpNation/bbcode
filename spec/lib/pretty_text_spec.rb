...
RSpec.describe PrettyText do
  ...
  it "uses one class namespace across separate custom blocks in a post" do
    fragment = bbcode_fragment(<<~BBCODE)
        [class=card]color: red;[/class]

        A **Markdown** paragraph between the declaration and its elements.
      ...
      BBCODE
    cards = fragment.css("div[class^='card__post-']")
    styles = fragment.css('template[data-bbcode-plus="class"]')

    expect(cards.map(&:text)).to eq(["First card", "Second card"])
    expect(cards.map { |card| card["class"] }.uniq.size).to eq(1)
    expect(styles.size).to eq(1)
    expect(styles.first.text).to include(".#{cards.first["class"]}", "color: red;")
    expect(fragment.at_css("strong").text).to eq("Markdown")
  end

  it "keeps class styles local to each cook" do
    first = bbcode_fragment("[class=card]color: red;[/class]\n[div class=card]First[/div]")
    second = bbcode_fragment("[class=card]color: blue;[/class]\n[div class=card]Second[/div]")
    first_card = first.at_css("div[class^='card__post-']")
    second_card = second.at_css("div[class^='card__post-']")
    first_styles = first.at_css('template[data-bbcode-plus="class"]').text
    second_styles = second.at_css('template[data-bbcode-plus="class"]').text

    expect(first_card["class"]).not_to eq(second_card["class"])
    expect(first_styles).to include(".#{first_card["class"]}", "color: red;")
    expect(second_styles).to include(".#{second_card["class"]}", "color: blue;")
    expect(second_styles).not_to include(first_card["class"])
    expect(bbcode_fragment("Ordinary Markdown").css("template")).to be_empty
  end

  it "preserves image references in both class declarations and inline styles" do
    remote_url = "https://example.test/card.png"
    upload_url = "upload://example-background.png"
    fragment = bbcode_fragment(<<~BBCODE)
        [class=card]background-image: url(#{remote_url});[/class]

        [div class=card style="background-image: url(#{upload_url})"]Card[/div]
      BBCODE

    expect(fragment.at_css('template[data-bbcode-plus="class"]').text).to include(remote_url)
    expect(fragment.at_css("div[class^='card__post-']")["style"]).to include(upload_url)
    candidates = HotlinkedMedia.extract_candidates(fragment)
    expect(candidates.map { |candidate| HotlinkedMedia.download_src_for(candidate) }).to eq(
      [remote_url],
    )
  end

  it "retains the font and layout attributes consumed by client decorators" do
    fragment = bbcode_fragment(<<~BBCODE)
        [font=Roboto]Custom font[/font]

        [fieldset=Details]Contents[/fieldset]

        [progress=50]Half complete[/progress]
      BBCODE

    expect(fragment.at_css("span[data-font]")["data-font"]).to start_with(
      "https://fonts.googleapis.com/css2?",
    )
    expect(fragment.at_css("fieldset.bb-fieldset > div.bb-fieldset").text).to eq("Contents")
    expect(fragment.at_css(".bb-progress > .bb-progress-bar-other")).to be_present
  end

  it "applies feature settings independently on consecutive cooks" do
    raw = "[center]Custom content[/center]\n\nA **Markdown** paragraph."
    enabled = bbcode_fragment(raw)
    SiteSetting.bbcode_enabled = false
    disabled = bbcode_fragment(raw)
    SiteSetting.bbcode_enabled = true
    reenabled = bbcode_fragment(raw)

    expect(enabled.at_css(".bb-center").text).to eq("Custom content")
    expect(disabled.css(".bb-center")).to be_empty
    expect(disabled.text).to include("[center]Custom content[/center]")
    expect(disabled.at_css("strong").text).to eq("Markdown")
    expect(reenabled.at_css(".bb-center").text).to eq("Custom content")
  end

  it "preserves custom cooking after the PrettyText engine is reset" do
    raw = "[spoiler=Example][b]Hidden text[/b][/spoiler]"
    first = bbcode_fragment(raw)
    PrettyText.reset_context
    second = bbcode_fragment(raw)

    expect(second.to_html).to eq(first.to_html)
    expect(second.at_css("details.bb-spoiler .bbcode-b").text).to eq("Hidden text")
    expect(bbcode_fragment("First paragraph.\n\nSecond paragraph.").css("p").map(&:text)).to eq(
